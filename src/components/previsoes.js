// ============================================================================
// OrionAI — Módulo de Previsões Inteligentes (Versão para React/Vite)
// ============================================================================
...
export function initPrevisoes() {
  ...
  const detectSeasonality = (data) => {
    if (data.length < 14) return null; // pelo menos 2 semanas
  ...
  const forecastWithSeasonality = (data, daysAhead = 5) => {
    const linear = forecastLinear(data, daysAhead);
    const seasonality = detectSeasonality(data);
    if (!seasonality) return linear;
  ...
  // ---------- Série sintética a partir do sentimento ----------
  const buildForecastSeriesFromTrend = (trendValue, days) => {
  const today = new Date();

  // Base da linha (pode calibrar depois)
  const base = 50;

  // A inclinação depende da força da tendência
  const slope = trendValue / 10;  
  // ex: +40 → slope = 4 / dia

  const historical = [];
  const forecast = [];

  // Criar histórico de 5 dias com leve oscilação institucional
  for (let i = -4; i <= 0; i++) {
    const d = new Date(today);
    d.setDate(d.getDate() + i);

    // Oscilação institucional
    const wave = Math.sin(i * 1.4) * 3;

    historical.push({
      date: d.toISOString().split('T')[0],
      count: Math.round(base + wave),
      isForecast: false
    });
  }

  const lastReal = historical[historical.length - 1].count;

  for (let i = 1; i <= days; i++) {
    const d = new Date(today);
    d.setDate(d.getDate() + i);

    // Projeção inclinada baseada na tendência
    let predicted = lastReal + slope * i;

    // Oscilação institucional suave
    predicted += Math.sin(i * 1.3) * 2;

    forecast.push({
      date: d.toISOString().split('T')[0],
      count: Math.max(0, Math.round(predicted)),
      isForecast: true
    });
  }
  ...
  const renderChart = (series, forecast = []) => {
    ...
    try {
      ...
      const isForecastArr = allData.map(p => !!p.isForecast || !!p.is_prediction);
          ...
          plugins: {
            ...
            tooltip: {
              ...
              callbacks: {
                title: (items) => {
                  const item = items[0];
                  const isPred = isForecastArr[item.dataIndex];
